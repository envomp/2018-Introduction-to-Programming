...
def get_competitors_list(filename: str) -> list:
    ...
    with open(filename, 'r') as file:
        for line in file:
            competitors.append(line[:-1:] if line[-1] == '\n' else line)
...
def get_results_dict(filename: str) -> dict:
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            results_dict[data[0]] = int(data[1])
...
def filter_results(path_to_competitors: str, path_to_results: str) -> dict:
    ...
    for name, result in results_dict.items():
        if name in competitors_list:
            corrected_dict[name] = result
...
def sort_results(path_to_competitors: str, path_to_results: str) -> list:
    ...
    corrected_dict = filter_results(path_to_competitors, path_to_results)

    for name, result in corrected_dict.items():
        temp_dict[name] = (result, competitors.index(name))

    temp_list = sorted(temp_dict.items(), key=lambda temp_dict: (-temp_dict[1][0], temp_dict[1][1]))

    for i in temp_list:
        sorted_list.append((i[0], i[1][0]))
...
def write_results_csv(path_to_competitors: str, path_to_results: str, file_to_write: str) -> None:
    ...
    # remove_last_line_from_csv(file_to_write)


def remove_last_line_from_csv(path_to_csv_file: str):
    """
    Remove empty line from the end of the CSV-file.
